...
__global__ void
somaFonteIntegral(float *Px, float *Pz, int X, int Z, int T, int t, int pixel_p_element, int n_fonte, int *pos_source_x, int *pos_source_z, float *source, float *integral, int flip, int idx=-1)
{
    const int indexF = threadIdx.x + blockDim.x*threadIdx.y + blockIdx.x*(BLOCK_SIZE*BLOCK_SIZE);
    const int n = indexF; 
...
void cuda_simulate(int en_out, int idx_source)
{
    ...
    {
	char mpegCom[500];
	sprintf(mpegCom, "ffmpeg -y -f rawvideo -vcodec rawvideo -pix_fmt gray -s %ix%i -r 60 -i - -f mp4 -q:v 5 -an -vcodec h264 -crf 0 output/outputPML_s%i.mp4 -nostats -loglevel quiet", X, Z, idx_source);
	...
	somaFonteIntegral<<<n_blocksF, threadGrid>>>(Px, Pz, X, Z, T, t, pixel_per_element, n_source, pos_source_x, pos_source_z, source, integral_source, 0, idx_source);
	cudaDeviceSynchronize();
...
cuda_grad_ext(float *mse_h, adj_func adj_calc, int idx_source)
{
	...
	// sprintf(mpegCom, "ffmpeg -y -f rawvideo -vcodec rawvideo -pix_fmt gray -s %ix%i -r 20 -i - -f mp4 -q:v 5 -an -vcodec h264 -crf 0 output/output_adj.mp4 -nostats -loglevel quiet", X, Z);    
	// pipeout = popen(mpegCom, "w");    
	...
	if(t>0)
	{
	    ...
	    somaFonteIntegral<<<n_blocksS, threadGrid>>>(Px_f, Pz_f, X, Z, T, t, pixel_per_element, n_sensor, pos_sensor_x, pos_sensor_z, adj_source, integral_adjsource, 1);
