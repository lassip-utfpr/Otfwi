...
outfilename = 'Setup4_OCNDT'

Lx = 1*150e-3 # Block width
Lz = 1*150e-3 # Block height
Lt = 180e-6 # Simulation time
...
draw_rectangle(params_real, 0.2*Nz, 0.2*Nx, value=cMat['acrylic'])
# draw_circle(params_real, 0.2*Nz, value=cMat['water'])

with open('output_wasserstein/modelmisfit_' + outfilename + '.pickle', 'rb') as f:
    [_, estimativa, _, _, _, _, _, _, _, mod_misfit_W2] = pickle.load(f)
